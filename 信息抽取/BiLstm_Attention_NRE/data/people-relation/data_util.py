...
word2id["BLANK"]=len(word2id)+1  #用于padding
...
#max length of position embedding is 40 (-40~+40)    
def pos(num):
    if num<-40:
        return 0
...
import pickle
with open('../people-relation/people_relation_train.pkl', 'wb') as outp:
...
import pickle
with open('../people-relation/people_relation_test.pkl', 'wb') as outp:
	pickle.dump(datas, outp)
	pickle.dump(labels, outp)
	pickle.dump(positionE1, outp)
	pickle.dump(positionE2, outp)
